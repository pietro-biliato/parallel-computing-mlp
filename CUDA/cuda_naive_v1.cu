...
/*
  KERNEL: Linear Forward Pass (Y = X * W^T + b) with fused ReLU.
  Thread Mapping:
  - Y axis (row) -> batch size (B)
  - X axis (col) -> output neurons (O)
  Memory Layout (row-major PyTorch):
  - X: [B, I] -> access: X[row * I + k]
  - W: [O, I] -> access: W[col * I + k] (W is already logically transposed by PyTorch)
  - b: [O]    -> access: b[col]
  - Y: [B, O] -> access: Y[row * O + col]
 */
...
int main() {
    ...
    // ==========================================
    /* ARCHITECTURAL CHOICE: block size 16x16 = 256 threads per block.
       256 is a multiple of 32 (warp size) --> attempt to avoid divergence or inactive threads in the warp,
       and it is low enough to allow allocating multiple blocks on a single SM 
       --> first attempt to maximize occupancy and hide memory latency.*/
    dim3 blockSize(16, 16);

    // Grid calculation for each layer (rounding up)
    dim3 gridSize1((O1 + blockSize.x - 1) / blockSize.x, (B + blockSize.y - 1) / blockSize.y);
    dim3 gridSize2((O2 + blockSize.x - 1) / blockSize.x, (B + blockSize.y - 1) / blockSize.y);
    dim3 gridSize3((O3 + blockSize.x - 1) / blockSize.x, (B + blockSize.y - 1) / blockSize.y);
    ...
    // Layer 1
    linear_forward_naive<<<gridSize1, blockSize>>>(d_X, d_W1, d_b1, d_H1, B, I1, O1, true);
    checkCuda(cudaGetLastError()); // Catch kernel launch errors

    // Layer 2
    linear_forward_naive<<<gridSize2, blockSize>>>(d_H1, d_W2, d_b2, d_H2, B, I2, O2, true);
    checkCuda(cudaGetLastError());

    // Layer 3
    linear_forward_naive<<<gridSize3, blockSize>>>(d_H2, d_W3, d_b3, d_Y, B, I3, O3, false);
    checkCuda(cudaGetLastError());
    ...
    const double PEAK_GFLOPS = 2550; // Peak FP32 for GTX 1650 Max-Q
    const double PEAK_BANDWIDTH = 112.0; // Peak bandwidth (GB/s)  
