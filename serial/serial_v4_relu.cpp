...
// V4: Applying the relu "within the forward pass"
std::vector<float> linear_layer_v4(const std::vector<float>& X, const std::vector<float>& W, const std::vector<float>& b, int B, int I, int O, bool apply_relu = true) {
    ...
    for (int i = 0; i < B; ++i) {
        ...
        for (int j = 0; j < O; ++j) {
            ...
            if (apply_relu) { // (this branch is perfectly predictable by the CPU --> will cause zero divergence of "threads")
                Y[i * O + j] = std::max(0.0f, sum); // ReLU applied
            } else {
                Y[i * O + j] = sum;
            }
        }
