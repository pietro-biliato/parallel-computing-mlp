...
std::vector<float> linear_layer_v3(const std::vector<float>& X, const std::vector<float>& W, const std::vector<float>& b, int B, int I, int O) {
    ...
    for (int i = 0; i < B; ++i) {
        // compute the starting address of the i-th row of X
        const float* x_row = &X[i * I]; 
        
        for (int j = 0; j < O; ++j) {
            // compute the starting address of the j-th row of W
            const float* w_row = &W[j * I]; 
            
            float sum = b[j]; 
            
            for (int k = 0; k < I; ++k) {
                sum += x_row[k] * w_row[k]; // only FMA float ops, zero index multiplications, that's the "trick"
            }
