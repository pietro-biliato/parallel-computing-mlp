...
std::vector<float> linear_layer_v0(const std::vector<float>& X, const std::vector<float>& W, const std::vector<float>& b, int B, int I, int O) {
    std::vector<float> Y(B * O, 0.0f); // RAM Initialization
    
    for (int i = 0; i < B; ++i) {
        for (int j = 0; j < O; ++j) {
            // continuous reading and WRITING in memory for each single calculation --> thus naive
            for (int k = 0; k < I; ++k) {
                Y[i * O + j] = Y[i * O + j] + X[i * I + k] * W[j * I + k];
            }
            // addition of bias with further memory access
            Y[i * O + j] = Y[i * O + j] + b[j];
        }
    }
    return Y;
...
int main(int argc, char* argv[]) {
    ...
    // INITIALIZATION
    // ==========================================
    // To automate execution and standardize result collection, this code receives 4 input arguments:
    // B, file version (v_0 in this case), whether to use -O0 or -03 to compile, how many runs to execute
    if (argc < 5) {
    ...
    /* ANALYTICAL FORMULAS:
       1. FLOPs:
          ...
          Each element of Y requires I multiplications and I additions (I-1 for the dot product + 1 for the bias) 
          --> 2*I FLOPs per element --> total per layer = B * O * 2*I.
          ReLU requires 1 FLOP (comparison) per element = B * O.
    */
    ...
    /*
       2. Memory Access (Bytes):
          We assume the ideal case (Cold Cache, we read everything from RAM once).
          Bytes = (Size(X) + Size(W) + Size(b) + Size(Y)) * 4 bytes (float32).
    */
    double bytes_l1 = (B*I1 + O1*I1 + O1 + B*O1) * 4.0;
    double bytes_l2 = (B*I2 + O2*I2 + O2 + B*O2) * 4.0;
    double bytes_l3 = (B*I3 + O3*I3 + O3 + B*O3) * 4.0;
    // Note: H1 and H2 are written and then read again, so we count them in their respective layers.
