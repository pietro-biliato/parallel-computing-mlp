...
// V1: Accumulator in Register (<-> minimizing cache writes)
std::vector<float> linear_layer_v1(const std::vector<float>& X, const std::vector<float>& W, const std::vector<float>& b, int B, int I, int O) {
    std::vector<float> Y(B * O, 0.0f);
    
    for (int i = 0; i < B; ++i) {
        for (int j = 0; j < O; ++j) {
            float sum = 0.0f; // the partial sum lives in a fast register
            
            for (int k = 0; k < I; ++k) {
                sum += X[i * I + k] * W[j * I + k]; // no RAM/Cache writes here!
            }
            
            Y[i * O + j] = sum + b[j]; // a single write per neuron
        }
    }
    return Y;
}
