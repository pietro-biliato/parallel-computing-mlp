...
// V4: Getting back to 'collapse(2) but applying the ReLu "within the forward pass"
std::vector<float> linear_layer_v4(const std::vector<float>& X, const std::vector<float>& W, const std::vector<float>& b, int B, int I, int O, bool apply_relu = true) {
    ...
    #pragma omp parallel for default(none) \
                             shared(X, W, b, Y, B, I, O, apply_relu) \
                             collapse(2) schedule(static)
    
    for (int i = 0; i < B; ++i) {
        for (int j = 0; j < O; ++j) {

            const float *x_row = &X[i * I]; // Recovered the perfect nesting
