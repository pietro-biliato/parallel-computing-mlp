...
// V3-OpenMP: Parallelization with collapse(1), keeping the micro-optimization on pointers arithmetic
std::vector<float> linear_layer_v3(const std::vector<float>& X, const std::vector<float>& W, const std::vector<float>& b, int B, int I, int O) {
    ...
    for (int i = 0; i < B; ++i) {
        const float *x_row = &X[i * I]; // Keeping the micro-optimization sacrificing 'collapse(2)'
        for (int j = 0; j < O; ++j) {
...
// In-place ReLU activation function: f(x) = max(0, x), PARALLELIZED
