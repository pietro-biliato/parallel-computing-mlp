...
// V1-OpenACC: the laziest possible approach to parallelizing on GPU:
// using a single 'kernel' directive and nothing else, but passing the arrays using 
// the 'restrict' keyword.
// No data structure used, not evven in the 'main' --> must compile with the flag '-gpu=mem:managed'
void linear_layer_v4_step1(const float* restrict X, 
    ...
    #pragma acc kernels
    {
        for (int i = 0; i < B; ++i) {
            for (int j = 0; j < O; ++j) {
                ...
                if (apply_relu) { // using ternary to avoid std::max host-device confusion
