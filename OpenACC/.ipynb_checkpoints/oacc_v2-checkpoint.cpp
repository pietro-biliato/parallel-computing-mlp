...
// V2: V1 refused to parallelize the outermost loop --> this is still the laziest, but more 
// parallelized version --> still to be compiled with '-gpu=mem:managed'
void linear_layer_v4_step2(const float* restrict X, 
    ...
    #pragma acc kernels
    {
        ...
        // It is promising the compiler it is safe to parallelize this loop --> it will be parallelized
        for (int i = 0; i < B; ++i) {
            
            for (int j = 0; j < O; ++j) {
                float sum = b[j]; 

                //#pragma acc loop reduction(+:sum) // Not needed, not even to prevent race conditions;
                // the compiler will prove to be smart enough to recognize the reduction by itself.
                // Again, lazy version
                for (int k = 0; k < I; ++k) {
                    sum += X[i * I + k] * W[j * I + k];
                }
                ...
                if (apply_relu) {
                    Y[i * O + j] = Y[i * O + j] = sum > 0.0f ? sum : 0.0f;
                } else {
                    Y[i * O + j] = sum;
                }
            }
...
int main() {
    ...
    std::cout << "   Total inferenza Tot. : " << std::setprecision(6) << time_sec * 1000.0 << " ms" << std::endl;
